...
class Datapoint extends EventEmitter {
  ...
  bind(conn) {
    ...
    if (this.options.autoread)
      ...
      conn.on("transition", (ev) => {
        if (ev.toState === "connected") {
          KnxLog.get().trace("======== READ INITIAL ======");
          try {
            this.read();
          } catch (e) {
            KnxLog.get().notice(
              "Unable to read group address %s",
              this.options.ga
            );
          }
        }
  ...
  update(jsvalue) {
    const oldValue = this.current_value;
    if (oldValue === jsvalue) return;
  ...
  async write(value) {
    if (!this.conn)
      throw new Error("must supply a valid KNX connection to bind to");
    if (Object.prototype.hasOwnProperty.call(this.dpt, "range")) {
    ...
    const result = await this.conn.write(this.options.ga, value, this.dptid);
    this.update(value);
    return result;
  }
  ...
  async read() {
    ...
    let jsvalue = null;
    try {
      const buf = await this.conn.read(this.options.ga);

      jsvalue = DPTLib.fromBuffer(buf, this.dpt);
    } catch (e) {
      KnxLog.get().info("Reading group address %s failed", this.options.ga);
    }
    return jsvalue;
  }
