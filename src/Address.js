...
const parse = function (
  ...
  const tokens = addr
    ...
    .map((w) => parseInt(w));
  if (tokens.length < 2)
    throw new Error("Invalid address (less than 2 tokens)");
  const [hinibble, midnibble, lonibble] = tokens;
  if (group && twoLevelAddressing) {
    // 2 level group address
    if (r(hinibble, 31))
      throw new Error("Invalid KNX 2-level main group: " + addr);
    if (r(midnibble, 2047))
      throw new Error("Invalid KNX 2-level sub group: " + addr);
    address.writeUInt16BE((hinibble << 11) + midnibble, 0);
    return address;
  }
  if (tokens.length < 3) throw new Error("Invalid address - missing 3rd token");
  if (group) {
    // 3 level group address
    if (r(hinibble, 31))
      throw new Error("Invalid KNX 3-level main group: " + addr);
    if (r(midnibble, 7))
      throw new Error("Invalid KNX 3-level mid group: " + addr);
    if (r(lonibble, 255))
      throw new Error("Invalid KNX 3-level sub group: " + addr);
    address.writeUInt8((hinibble << 3) + midnibble, 0);
    address.writeUInt8(lonibble, 1);
    return address;
  }
  // 3 level physical address
  if (r(hinibble, 15)) throw new Error("Invalid KNX area address: " + addr);
  if (r(midnibble, 15)) throw new Error("Invalid KNX line address: " + addr);
  if (r(lonibble, 255)) throw new Error("Invalid KNX device address: " + addr);
